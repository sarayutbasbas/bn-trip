...
export function getDemoTrips(params:URLSearchParams){
  ...
  const filteredScope=trips.filter(trip=>(tripType==="all"||(tripType==="domestic"&&trip.country_code==="TH")||(tripType==="international"&&Boolean(trip.country_code)&&trip.country_code!=="TH"))&&(!years.length||years.includes(Number(trip.start_date.slice(0,4))))&&(!search||`${trip.name} ${trip.destination}`.toLowerCase().includes(search)));
