...
export function getDemoTrips(params:URLSearchParams){
  ...
  let items=trips.filter(trip=>(status==="all"||(status==="ongoing"&&ongoing.includes(trip))||(status==="upcoming"&&upcoming.includes(trip))||(status==="past"&&past.includes(trip)))&&(tripType==="all"||(tripType==="domestic"&&trip.country_code==="TH")||(tripType==="international"&&Boolean(trip.country_code)&&trip.country_code!=="TH"))&&(!year||Number(trip.start_date.slice(0,4))===year)&&(!search||`${trip.name} ${trip.destination}`.toLowerCase().includes(search)));
