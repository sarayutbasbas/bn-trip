...
export function getDemoTrips(params:URLSearchParams){
  ...
  const status=params.get("status")||"all",tripType=params.get("type")||"all",year=Number(params.get("year")||0),search=(params.get("q")||"").trim().toLowerCase(),sort=params.get("sort")||"latest",limit=Math.min(50,Math.max(1,Number(params.get("limit")||20))),offset=Math.max(0,Number(params.get("offset")||0));
  const filteredScope=trips.filter(trip=>(tripType==="all"||(tripType==="domestic"&&trip.country_code==="TH")||(tripType==="international"&&Boolean(trip.country_code)&&trip.country_code!=="TH"))&&(!year||Number(trip.start_date.slice(0,4))===year)&&(!search||`${trip.name} ${trip.destination}`.toLowerCase().includes(search)));
