...
function buildTrips(){
  ...
  return [
    {id:"d1000000-0000-4000-8000-000000000001",owner_id:DEMO_USER_ID,name:"Kyoto Slow Days",destination:"Kyoto, Japan",country_code:"JP",trip_destinations:[{id:"JP:kyoto",countryCode:"JP",nameTh:"เกียวโต",nameEn:"Kyoto",badgeId:"japan:kyoto"}],start_date:ongoingStart,total_days:5,traveller_count:2,budget_thb:"45000.00",shopping_budget_thb:"12000.00",actual_spent_thb:4138.2,outbound_departure_at:timestamp(ongoingStart,"08:30"),return_departure_at:timestamp(ongoingEnd,"19:00"),cover_image_url:"/travel-postcard-background.jpg",google_photos_url:null,access_role:"owner",members:[]},
    {id:"d1000000-0000-4000-8000-000000000002",owner_id:DEMO_USER_ID,name:"Fukuoka Food Trip",destination:"Fukuoka, Japan",country_code:"JP",trip_destinations:[{id:"JP:fukuoka",countryCode:"JP",nameTh:"ฟุกุโอกะ",nameEn:"Fukuoka",badgeId:"japan:fukuoka"}],start_date:upcomingStart,total_days:5,traveller_count:2,budget_thb:"60000.00",shopping_budget_thb:"15000.00",actual_spent_thb:1280.4,outbound_departure_at:timestamp(upcomingStart,"07:15"),return_departure_at:timestamp(upcomingEnd,"21:20"),cover_image_url:"/travel-postcard-fallback.jpg",google_photos_url:null,access_role:"owner",members:[]},
    {id:"d1000000-0000-4000-8000-000000000003",owner_id:DEMO_USER_ID,name:"Hokkaido Winter Memory",destination:"Hokkaido, Japan",country_code:"JP",trip_destinations:[{id:"JP:hokkaido",countryCode:"JP",nameTh:"ฮอกไกโด",nameEn:"Hokkaido",badgeId:"japan:hokkaido"}],start_date:pastStart,total_days:4,traveller_count:2,budget_thb:"80000.00",shopping_budget_thb:"20000.00",actual_spent_thb:121,outbound_departure_at:timestamp(pastStart,"09:00"),return_departure_at:timestamp(pastEnd,"18:30"),cover_image_url:"/travel-postcard-background.jpg",google_photos_url:null,access_role:"owner",members:[],review_average:5,review_count:1},
  ];
}
...
export function getDemoTrips(params:URLSearchParams){
  const trips=buildTrips().map(trip=>({...trip,flight_summaries:getDemoFlightSegments(trip.id).map(flight=>({journey_type:flight.journey_type,segment_order:flight.segment_order,airline_code:flight.airline_code,flight_number:flight.flight_number}))}));const now=Date.now();const departure=(trip:(typeof trips)[number])=>new Date(trip.outbound_departure_at).getTime();const arrival=(trip:(typeof trips)[number])=>new Date(trip.return_departure_at).getTime();const ongoing=trips.filter(trip=>departure(trip)<=now&&arrival(trip)>=now);const upcoming=trips.filter(trip=>departure(trip)>now);const past=trips.filter(trip=>arrival(trip)<now).sort((a,b)=>arrival(b)-arrival(a));
  if(params.get("mode")==="dashboard")return {ongoing,upcoming,past:past.slice(0,8),plans:trips.filter(trip=>"summary_image_url" in trip&&Boolean(trip.summary_image_url)),counts:{total:trips.length,ongoing:ongoing.length,upcoming:upcoming.length,past:past.length,countries:new Set(past.map(trip=>trip.country_code||trip.destination).filter(Boolean)).size,destinations:new Set(past.flatMap(trip=>trip.trip_destinations?.map(destination=>destination.id)||[trip.destination.split(",")[0].trim().toLowerCase()])).size,travel_days:past.reduce((total,trip)=>total+Number(trip.total_days||0),0)},countryHighlights:[{countryCode:"JP",country:"Japan",trips:past.length,averageRating:5,reviewCount:1}],years:[...new Set(trips.map(trip=>Number(trip.start_date.slice(0,4))))].sort((a,b)=>b-a)};
  if(params.get("mode")!=="list")return trips;
  const status=params.get("status")||"all",tripType=params.get("type")||"all",years=[...new Set(params.getAll("year").flatMap(value=>value.split(",")).map(Number).filter(year=>Number.isInteger(year)&&year>=2000&&year<=2200))],search=(params.get("q")||"").trim().toLowerCase(),sort=params.get("sort")||"latest",limit=Math.min(50,Math.max(1,Number(params.get("limit")||20))),offset=Math.max(0,Number(params.get("offset")||0));
  const filteredScope=trips.filter(trip=>{const country=countryByCode(trip.country_code);const searchable=`${trip.name} ${trip.destination} ${country?.nameTh||""} ${country?.nameEn||""} ${(country?.aliases||[]).join(" ")} ${trip.country_code||""}`.toLowerCase();return (tripType==="all"||(tripType==="domestic"&&trip.country_code==="TH")||(tripType==="international"&&Boolean(trip.country_code)&&trip.country_code!=="TH"))&&(!years.length||years.includes(Number(trip.start_date.slice(0,4))))&&(!search||searchable.includes(search))});
  let items=filteredScope.filter(trip=>status==="all"||(status==="ongoing"&&ongoing.includes(trip))||(status==="upcoming"&&upcoming.includes(trip))||(status==="past"&&past.includes(trip)));
  ...
  return {items:items.slice(offset,offset+limit),total:items.length,years:[...new Set(trips.map(trip=>Number(trip.start_date.slice(0,4))))].sort((a,b)=>b-a),hasMore:offset+limit<items.length,statusCounts:{all:filteredScope.length,ongoing:filteredScope.filter(trip=>ongoing.includes(trip)).length,upcoming:filteredScope.filter(trip=>upcoming.includes(trip)).length,past:filteredScope.filter(trip=>past.includes(trip)).length}};
}
