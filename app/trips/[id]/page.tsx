...
export default async function TripPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{returnTo?:string|string[];workspace?:string|string[];view?:string|string[]}>}){
  ...
  const tripView=query.view==="flights"?"flights":undefined;
  ...
  return <BNTripApp authenticated demo={Boolean(session.isDemo)} page="trip" tripId={id} returnTo={returnTo} workspaceTab={workspaceTab} tripView={tripView} initialTrip={initialTrip as Trip|null} initialItineraries={initialItineraries as Itinerary[]} initialTripCards={initialTripCards as PaymentCard[]}/>;
}
