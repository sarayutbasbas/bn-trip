...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    const result=await query("UPDATE itineraries i SET day_number=$1,time_slot=$2,start_time=$3,place_name=$4,address=$5,image_added_at=CASE WHEN $6 AND $7::text IS NULL THEN NULL WHEN $6 AND i.image_url IS NULL THEN now() ELSE i.image_added_at END,image_url=CASE WHEN $6 THEN $7 ELSE i.image_url END,transport_mode=$8,transport_note=$9,cost_items=$10::jsonb,updated_at=now() FROM trips t WHERE i.id=$11 AND t.id=i.trip_id AND $1 BETWEEN 1 AND t.total_days RETURNING i.*",[x.dayNumber,x.timeSlot,x.startTime,x.placeName,x.address||null,hasImageUrl,x.imageUrl??null,x.transportMode||null,x.transportNote||null,JSON.stringify(costItems),id]);
    if(!result.rows[0])return NextResponse.json({error:"Not found"},{status:404});await clearFirstItineraryTransport(existing.trip_id,[existing.day_number,x.dayNumber]);await syncAccommodationCostsFromItineraries(existing.trip_id);const saved=await query(`SELECT i.*,
      ...
      LEFT JOIN LATERAL (
        ...
        ORDER BY candidate.image_added_at ASC NULLS LAST,candidate.id LIMIT 1
