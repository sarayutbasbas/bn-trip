...
export async function DELETE(_:Request,{params}:{params:Promise<{id:string}>}){
  ...
  if(session.isDemo)return NextResponse.json({error:"Demo mode is read-only",loginRequired:true},{status:403});
  const {id}=await params;
  const trip=await query<{trip_id:string}>("SELECT trip_id FROM itineraries WHERE id=$1",[id]);const role=trip.rows[0]?await getTripRole(trip.rows[0].trip_id,session.userId):null;if(role!=="owner"&&role!=="admin")return NextResponse.json({error:"สิทธิ์ View ไม่มีสิทธิลบรายการ"},{status:403});
