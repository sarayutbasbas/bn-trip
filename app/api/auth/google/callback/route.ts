...
export async function GET(request:Request){
  ...
  try{
    ...
    const email=normalizeEmail(payload.email);const ownerEmail=normalizeEmail(process.env.GOOGLE_OWNER_EMAIL||"sarayutkongpeng@gmail.com");
    const user=await transaction(async client=>{
      const invited=await client.query("SELECT 1 FROM trip_collaborators WHERE lower(email)=$1 LIMIT 1",[email]);
      if(email!==ownerEmail&&!invited.rowCount)throw new Error("not_invited");
      let existing=await client.query<{id:string}>("SELECT id FROM users WHERE google_sub=$1 OR lower(email)=$2 ORDER BY google_sub=$1 DESC LIMIT 1",[payload.sub,email]);
      if(!existing.rowCount&&email===ownerEmail)existing=await client.query<{id:string}>("SELECT id FROM users WHERE lower(email)=$1 LIMIT 1",[ownerEmail]);
      ...
      const row=existing.rows[0]
        ...
        :(await client.query<{id:string;email:string;display_name:string;avatar_url:string|null}>("INSERT INTO users(email,google_sub,display_name) VALUES($1,$2,$3) RETURNING id,email,display_name,avatar_url",[email,payload.sub,displayName])).rows[0];
  ...
  }catch(error){console.error("Google OAuth callback failed",error);return redirect(error instanceof Error&&error.message==="not_invited"?"not_invited":"google_login_failed")}
}
