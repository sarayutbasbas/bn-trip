...
const createSchema=z.discriminatedUnion("kind",[
  z.object({kind:z.literal("category"),name:z.string().trim().min(1).max(120),iconKey:z.enum(CHECKLIST_CATEGORY_ICON_KEYS).optional()}),
...
export async function GET(){
  ...
  const [categories,items]=await Promise.all([
    query("SELECT id,name,icon_key,sort_order,created_at FROM checklist_master_categories WHERE user_id=$1 ORDER BY sort_order,created_at",[session.userId]),
...
export async function POST(request:Request){
  ...
  try{const input=createSchema.parse(await request.json());
    if(input.kind==="category"){const result=await query(`INSERT INTO checklist_master_categories (user_id,name,icon_key,sort_order) VALUES ($1,$2,$3,COALESCE((SELECT max(sort_order)+1 FROM checklist_master_categories WHERE user_id=$1),0)) RETURNING *`,[session.userId,input.name,input.iconKey||inferChecklistCategoryIcon(input.name)]);return NextResponse.json(result.rows[0],{status:201})}
