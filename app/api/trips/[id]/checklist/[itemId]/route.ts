...
export async function PATCH(
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "แก้ไขได้เฉพาะรายการที่คุณเพิ่ม" },
        { status: 403 },
      );
    ...
    const updated = await transaction(async (client) => {
      const category = input.categoryId
        ...
        : null;
      if (input.categoryId && !category?.rowCount)
      ...
      const result = await client.query<ChecklistRow>(
        ...
        completed_by=CASE WHEN $5::boolean IS NULL THEN completed_by WHEN $5 THEN $6::uuid ELSE NULL END,updated_at=now()
        WHERE id=$7 AND trip_id=$8 RETURNING *`,
        [
          input.title || null,
          category?.rows[0]?.name || null,
      ...
      if ((input.title || input.categoryId) && before.rows[0].master_item_id) {
        await client.query(
...
export async function DELETE(
  ...
  const result = await query(
    "DELETE FROM trip_checklist_items WHERE id=$1 AND trip_id=$2 AND created_by=$3 RETURNING *",
    [itemId, id, session.userId],
  );
  if (!result.rowCount)
    return NextResponse.json(
      { error: "ลบได้เฉพาะรายการที่คุณเพิ่ม" },
      { status: 403 },
    );
  await logTripActivity({
    tripId: id,
