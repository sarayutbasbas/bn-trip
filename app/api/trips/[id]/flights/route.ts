...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
  ...
  return NextResponse.json({segments:result.rows,referenceData,syncConfigured:Boolean(process.env.FLIGHTAWARE_API_KEY),documentUploadMode:getStorageBackend()==="blob"?"client":"server"});
}
...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    const segment=await transaction(async client=>{
      ...
      await client.query("UPDATE trips SET has_flights=true,updated_at=now() WHERE id=$1",[id]);
      await syncTripDayZero(client,id);
