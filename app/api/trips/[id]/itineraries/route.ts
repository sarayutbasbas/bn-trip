...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    ...
    return result.rows[0]?NextResponse.json(result.rows[0],{status:201}):NextResponse.json({error:"Trip not found or day is outside the trip"},{status:404});
