...
import { removeAllFlightRecords } from "@/src/lib/flight-linked-records";
...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const before=await query("SELECT * FROM trips WHERE id=$1",[id]);const result=await transaction(async client=>{if(body.hasFlights===false){const affectedDays=await removeAllFlightRecords(client,id);await clearFirstItineraryTransport(id,affectedDays,client)}return client.query(`UPDATE trips SET name=COALESCE($1,name),destination=COALESCE($2,destination),country_code=$3,country_name=$4,start_date=$5,total_days=$6,budget_thb=COALESCE($7,budget_thb),shopping_budget_thb=COALESCE($8,shopping_budget_thb),outbound_departure_at=$9,return_departure_at=$10,cover_image_url=COALESCE($11,cover_image_url),google_photos_url=$12,timezone=$13,has_flights=COALESCE($14,has_flights),updated_at=now() WHERE id=$15 RETURNING *,CASE WHEN owner_id=$16 THEN 'owner' ELSE COALESCE((SELECT access_level FROM trip_collaborators WHERE trip_id=$15 AND user_id=$16 LIMIT 1),'view') END AS access_role`,[body.name??null,destination||null,country.code,country.nameEn,body.outboundDate,totalDays,body.budgetThb??null,body.shoppingBudgetThb??null,`${body.outboundDate} ${body.outboundTime}:00`,`${body.returnDate} ${body.returnTime}:00`,body.coverImageUrl??null,googlePhotosUrl||null,country.timezone,typeof body.hasFlights==="boolean"?body.hasFlights:null,id,session.userId])});
