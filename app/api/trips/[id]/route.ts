...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const parsedNote=body.note===undefined?null:tripNoteSchema.safeParse(body.note);
  if(parsedNote&&!parsedNote.success)return NextResponse.json({error:"โน้ตต้องมีความยาวไม่เกิน 500 ตัวอักษร"},{status:400});
