...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const locationIds=Array.isArray(body.locationIds)?body.locationIds.filter((value:unknown):value is string=>typeof value==="string"):[];
  const tripDestinations=resolveTripDestinations(country.code,locationIds);if(!locationIds.length||tripDestinations.length!==new Set(locationIds).size)return NextResponse.json({error:"กรุณาเลือกเมืองจากรายการ"},{status:400});
  const destination=formatTripDestination(tripDestinations.map(item=>item.nameEn).join(" · "),country.code,country.nameEn);
  const before=await query("SELECT * FROM trips WHERE id=$1",[id]);const changed=await transaction(async client=>{let insuranceDocuments:Array<{stored_filename:string;blob_url:string|null}>=[];if(body.hasFlights===false){const removed=await removeAllFlightRecords(client,id);insuranceDocuments=removed.insuranceDocuments;await clearFirstItineraryTransport(id,removed.affectedDays,client)}const updated=await client.query(`UPDATE trips SET name=COALESCE($1,name),destination=COALESCE($2,destination),country_code=$3,country_name=$4,trip_destinations=$5::jsonb,start_date=$6,total_days=$7,budget_thb=COALESCE($8,budget_thb),shopping_budget_thb=COALESCE($9,shopping_budget_thb),outbound_departure_at=$10,return_departure_at=$11,cover_image_url=COALESCE($12,cover_image_url),google_photos_url=$13,timezone=$14,has_flights=COALESCE($15,has_flights),updated_at=now() WHERE id=$16 RETURNING *,CASE WHEN owner_id=$17 THEN 'owner' ELSE COALESCE((SELECT access_level FROM trip_collaborators WHERE trip_id=$16 AND user_id=$17 LIMIT 1),'view') END AS access_role`,[body.name??null,destination||null,country.code,country.nameEn,JSON.stringify(tripDestinations),body.outboundDate,totalDays,body.budgetThb??null,body.shoppingBudgetThb??null,`${body.outboundDate} ${body.outboundTime}:00`,`${body.returnDate} ${body.returnTime}:00`,body.coverImageUrl??null,googlePhotosUrl||null,country.timezone,typeof body.hasFlights==="boolean"?body.hasFlights:null,id,session.userId]);if(updated.rows[0])updated.rows[0].has_day_zero=await syncTripDayZero(client,id);return {updated,insuranceDocuments}});const result=changed.updated;
