  ...
  try {
    ...
    await transaction(async (client) => {
      ...
      const updated = await client.query<{ id: string; cost_item_id: string }>(`UPDATE trip_accommodations SET
        name=$3,location=$4,description=$5,check_in_day=$6,check_out_day=$7,check_in_time=$8::time,
        check_out_time=$9::time,foreign_amount=$10,currency=$11,exchange_rate=$12,
        rate_date=$13,payment_method=$14,credit_card_id=$15,payment_owner_name=$16,
        split_member_ids=$17::uuid[],updated_at=now()
        WHERE id=$1 AND trip_id=$2 RETURNING id,cost_item_id`, [accommodationId,id,input.name,input.location,input.description,input.checkInDay,input.checkOutDay,input.checkInTime,input.checkOutTime,input.foreignAmount,input.currency.toUpperCase(),input.exchangeRate,input.rateDate,input.paymentMethod,input.creditCardId||null,input.paymentOwnerName||null,input.splitMemberIds]);
