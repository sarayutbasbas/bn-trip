...
type FinalizeInput={title?:unknown;originalFilename?:unknown;mimeType?:unknown;size?:unknown;blobUrl?:unknown;pathname?:unknown};

async function insertDocument(input:{tripId:string;title:string;storedFilename:string;blobUrl:string|null;originalFilename:string;mimeType:string;size:number;userId:string}){
  return transaction(async client=>{
    ...
    const result=await client.query(`INSERT INTO trip_documents (trip_id,title,stored_filename,blob_url,original_filename,mime_type,file_size,uploaded_by)
      VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id,trip_id,title,original_filename,mime_type,file_size,created_at`,[input.tripId,input.title,input.storedFilename,input.blobUrl,input.originalFilename,input.mimeType,input.size,input.userId]);
...
export async function POST(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  try{
    let title:string,originalFilename:string,mimeType:string,size:number,storedFilename:string,blobUrl:string|null=null;
    if(isJson){
      ...
      title=String(body.title||"").trim();originalFilename=String(body.originalFilename||"").trim();mimeType=String(body.mimeType||"");size=Number(body.size);blobUrl=String(body.blobUrl||"");const pathname=String(body.pathname||"");
      ...
      const form=await request.formData();const file=form.get("file");title=String(form.get("title")||"").trim();
    ...
    const item=await insertDocument({tripId:id,title,storedFilename,blobUrl,originalFilename,mimeType,size,userId:session.userId});orphan=null;
