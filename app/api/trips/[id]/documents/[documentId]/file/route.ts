...
export async function GET(request:Request,{params}:{params:Promise<{id:string;documentId:string}>}){
  ...
  const file=await readUpload(item.stored_filename,request.headers.get("if-none-match")||undefined,item.blob_url);if(!file)return NextResponse.json({error:"Not found"},{status:404});const safeName=item.original_filename.replace(/[\r\n"]/g,"_");const headers:Record<string,string>={"Content-Type":file.contentType||item.mime_type,"Content-Disposition":`inline; filename="${safeName}"`,"Cache-Control":"private, no-cache","X-Content-Type-Options":"nosniff"};if(file.etag)headers.ETag=file.etag;if(file.statusCode===304)return new Response(null,{status:304,headers});return new Response(file.body,{headers});
}
