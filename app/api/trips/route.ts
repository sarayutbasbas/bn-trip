...
export async function GET(request:Request) {
  ...
  const result = await query(`SELECT t.*,${role},${members} FROM trips t WHERE ${access} ORDER BY t.start_date DESC`,[session.userId]); return NextResponse.json(result.rows);
}
